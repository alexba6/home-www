...
import { AuthKeyStore} from "../../Store/AuthKey/AuthKeyReducer";
...
export const AccountAuthKeyTab: FunctionComponent = () => {
	...
	return (
		<SettingsCard>
			<SettingsCardHeader title="Authentification" details="Ces sessions sont actuellement connectées à votre compte."/>
			<SettingsCardContent>
				<Table>
					<TableHead>
						<TableRow>
					...
					<TableBody>
						{authKeys.map((authKey: AuthKeyStore, key: number) => <TableRow key={key}>
							...
							<TableCell align='center'>
								{authKey.authKey.id !== authContext.authenticationKey.id && <Tooltip title='Supprimer cette connexion'>
